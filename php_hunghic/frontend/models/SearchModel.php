<?php 
	trait SearchModel{
		...
		public function modelReadSearchProductName($key,$recordPerPage){
			...
			//thuc hien truy van
			$query = $conn->query("select * from products where name like '%$key%' and price<500000 order by id desc limit $from,$recordPerPage");
		...
		public function modelBrandSearch($key){
			//lay bien ket noi
			$conn = Connection::getInstance();
			$query = $conn->query("select products.id as idd, photo , price,products.name as namee, discount from products,brands where brands.id=products.brand_id and brands.name = '$key' ");
			return $query->fetchAll();
		}	
	}
 ?>